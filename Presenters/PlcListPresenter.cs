...
namespace PLC.Presenters {
    ...
    public class PlcListPresenter {
        ...
        public PlcListPresenter (IPlcListView view, PlcStore store, IPlcFactory plcFactory) {
            ...
            // 数据变了就刷新列表显示
            _store.Changed += (s, e) => _view.IShowPlcList(_store.Plcs);
        }
        ...
        public void Initialize () {
            _view.IShowPlcList(_store.Plcs);
        }
        ...
        private void OnDeletePlc (object sender, EventArgs e) {
            int idx = _view.ISelectedPlcIndex;
            if (idx < 0) { _view.ILog("请先选中要删除的PLC"); return; }

            var config = _store.GetAt(idx);
            if (!_view.IConfirm($"确定删除 [{config.Link.PlcName}]？\n此操作不可恢复。", "删除确认"))
                return;
            ...
            // 删除后重新定位选中行
            if (_store.Count > 0)
                _view.ISelectPlc(Math.Min(idx, _store.Count - 1));

            _view.ILog($"🗑️ 已删除 PLC: {config.Link.PlcName}");
        }
        ...
        private void OnApplyIp (object sender, EventArgs e) {
            //! 选择校验
            int idx = _view.ISelectedPlcIndex;
            if (idx < 0) { _view.ILog("请先选中PLC"); return; }
            var config = _store.GetAt(idx);

            //! IP校验
            string newIp = _view.IIpText.Trim();
            if (!IsValidIp(newIp)) {
                _view.ILog($"IP格式无效: {newIp}"); return;
            }

            //! 连接校验
            if (config.Link.Client != null) {
                _view.ILog("请先断开连接后再修改IP"); return;
            }
            //! 应用修改
            config.Link.Ip = newIp;
            _store.NotifyChanged();             //! 属性数据, 会自动触发UI刷新
            _view.ILog($"✅ [{config.Link.PlcName}] IP已更新为 {newIp}");
        }
        ...
        private void OnApplyPort (object sender, EventArgs e) {
            ...
            //! 端口校验
            if (!int.TryParse(_view.IPortText.Trim(), out int port) || port <= 0 || port > 65535) {
                _view.ILog("端口必须是 1~65535 的整数"); return;
            }

            //! 连接校验
            if (config.Link.Client != null) {
                _view.ILog("请先断开连接后再修改端口"); return;
            }

            //! 应用修改
            config.Link.Port = port;
            _store.NotifyChanged();             //! 属性数据, 会自动触发UI刷新
            _view.ILog($"✅ [{config.Link.PlcName}] 端口已更新为 {port}");
        }
        ...
        private void OnProtocolChange (object sender, EventArgs e) {
            ...
            _store.NotifyChanged();             //! 属性数据, 会自动触发UI刷新
            _view.ILog($"✅ [{config.Link.PlcName}] 协议已更新为 {newProto}");
        }
        ...
        private void Move (int direction) {
            ...
            _view.ISelectPlc(newIdx); // 让选中跟着移动
        }
